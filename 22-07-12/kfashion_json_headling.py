...
# list for labels
colorList, topSleLengthList, bottomLengthList, opsLengthList, outerLengthList, printPatternList = list(), list(), list(), list(), list(), list()

# to count num of values
num = 0

# json file load
for i, allJson in enumerate(allJsonList[1450:1460]):
    ...
    topColorLabel, btmColorLabel, topLabel, bottomLabel, outerLabel, opsLabel, topPrintPatternLabel, btmPrintPatternLabel = "","","","","","","",""
    ...
    for k, v in jsonData['데이터셋 정보']['데이터셋 상세설명']['라벨링'].items():
        ...
        if k == '원피스' and cat not in ['드레스']:
            continue     
        if k in ['상의', '아우터', '원피스'] and printPattern not in ['무지', '체크', '스트라이프']:
            continue            
        if k == '원피스' and length in ['니렝스', '미디']:
            length = '미니'

        # topColorLabel
        if k == '상의' and cat in ['블라우스', '티셔츠', '셔츠']:
            topColorLabel = f"{k}_{color}"
            # if not topColorLabel in colorList:
            #     colorList.append(topColorLabel)
        ...
        # topPrintPatternLabel(printPattern)
        if k == '상의' and printPattern in ['무지', '체크', '스트라이프']:
            topPrintPatternLabel = f"{k}_{printPattern}"
            # if not topPrintPatternLabel in printPatternList:
            #     printPatternList.append(topPrintPatternLabel)
    ...
    # for k, v1 in labelingDict.items():
    #     if colorLabel == k or topLabel == k or outerLabel == k or opsLabel == k or printPatternLabel == k:
    #         v1 += 1
    ...
    for k, v in jsonData['데이터셋 정보']['데이터셋 상세설명']['라벨링'].items():
        ...
        if k == '하의' and cat == '청바지':
            cat = "팬츠"
        if k == '하의' and length == '맥시':
            length = '발목'
        if k == '하의' and cat not in ['팬츠', '스커트']:
            continue            
        if k == '하의' and printPattern not in ['무지', '체크', '스트라이프']:
            continue            
        if k == '하의' and length in ['니렝스', '미디']:
            length = '미니'
        
        # btmColorLabel(color)
        if k == '하의' and cat in ['팬츠', '스커트']:
            btmColorLabel = f"{k}_{color}"
            # if not btmColorLabel in colorList:
            #     colorList.append(btmColorLabel)
        ...
        # btmPrintPatternLabel(printPattern))        
        if k == '하의' and printPattern in ['무지', '체크', '스트라이프']:
            btmPrintPatternLabel = f"{k}_{printPattern}"
            # if not btmPrintPatternLabel in printPatternList:
            #     printPatternList.append(btmPrintPatternLabel)

    for k, v in labelingDict.items():
        if btmColorLabel == k or bottomLabel == k or btmPrintPatternLabel == k:
            v += 1
            print("btm", v)
        if topColorLabel == k or topLabel == k or outerLabel == k or opsLabel == k or topPrintPatternLabel == k:
            v += 1
            print("top", v)
        print(k, v)
