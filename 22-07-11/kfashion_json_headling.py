...
for i, allJson in enumerate(allJsonList[1450:1460]):
    ...
    colorLabel, topLabel, bottomLabel, outerLabel, opsLabel, printPatternLabel = "","","","","",""
    ...
    for k, v in jsonData['데이터셋 정보']['데이터셋 상세설명']['라벨링'].items():
        ...
        if k == '상의' and printPattern in ['무지', '체크', '스트라이프']:
            printPatternLabel = f"{k}_{printPattern}"
    ...
    for k, v in labelingDict.items():
        if colorLabel == k or bottomLabel == k or printPatternLabel == k:
            v += 1
        print(k, v)
    print("\n")
